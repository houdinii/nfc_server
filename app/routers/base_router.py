...
class BaseRouter(ABC):
    ...
    @property
    @abstractmethod
    def model_class(self) -> Type[DBBaseModel]:
    ...
    def _setup_routes(self):
        """Setup common CRUD routes if needed by subclasses"""
        pass
    ...
    def _get_user_entity(
        ...
        entity = db.query(self.model_class).filter(
            and_(
                ...
                self.model_class.user_id == current_user.id
            )
        ).first()
    ...
    def _delete_entity(self, entity: DBBaseModel, db: Session) -> None:
        """
        Delete an entity
...
class CRUDRouter(BaseRouter):
    ...
    def _setup_create_route(self):
        """Setup POST route for entity creation"""
    ...
    def _setup_list_route(self):
        """Setup GET route for listing entities"""
    ...
    def _setup_update_route(self):
        """Setup PATCH route for updating entities"""
    ...
    # Handler methods that can be overridden by subclasses
